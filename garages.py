...
def create_garage_(name: str, capacity: int):
    ...
    if capacity > 20:
        return "Maximum capacity of a garage is 20!"
...
def add_car_(brand: str, model: str, prod_year: int, garage_id: int ):
    ...
    if cars_in_garage < capacity and 1886 <= prod_year <= current_year + 1 and len(brand) > 0 and len(model) > 0:
    ...
    elif cars_in_garage < capacity and prod_year < 1886 or prod_year > current_year + 1:
